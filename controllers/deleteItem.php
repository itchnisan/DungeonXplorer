<?php
require_once '../models/Hero.php';
require_once '../models/Treasure.php';
require_once '../models/Chapter.php';
require_once '../models/Inventory.php';
require_once '../models/Items.php';
include_once '../models/connexionPDO.php';

session_start(); 

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Récupérer les données du formulaire
    $itemId = $_POST['item_id'] ?? null;
    $heroId = $_POST['hero_id'] ?? null;
    $inventory_id = $_POST['inventory_id'] ?? null;

    if ($itemId && $heroId) {
        $db = OuvrirConnexionPDO('mysql:host=localhost;dbname=dx_10;charset=utf8', 'root', '');

        // Préparer la requête SQL pour supprimer l'item de l'inventaire
        $sql = $db->prepare("DELETE FROM inventory WHERE hero_id = :hero_id AND items_id = :items_id and inventory_id = :inventory_id");
        $sql->execute([
            'hero_id' => $heroId,
            'items_id' => $itemId,
            'inventory_id' => $inventory_id
        ]);

        // Rediriger ou afficher un message après la suppression
        header('Location: ../Chapter/' . $_POST['chapter_id']); // Redirection vers la page du chapitre
        exit;
    } else {
        echo "Erreur : données invalides.";
    }
}
?>